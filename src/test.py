...
#            free_energy = - entropy / args.beta + energy0
        ...
        loss = torch.mean(- entropy * (energy0 - energy0.mean()))

        print('loss: {}, energy: {}'.format(loss, energy0.mean()/args.D))
        # backprop ang update
        opt.zero_grad()
        loss.backward()
        opt.step()
...
#    free_energy = - entropy / args.beta + energy0
    ...
    return entropy.mean().cpu().numpy(), config, energy, nums
...
if __name__ == '__main__':
    ...
    entropy, config, energy, nums = VAN(args)
